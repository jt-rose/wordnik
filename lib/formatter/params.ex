defmodule Wordnik.Formatter.Params do
  ...
  # convert atoms to strings to support atom or string-based map keys
  defp standardize_param({name, value}) when is_atom(name) do
    {Atom.to_string(name), value}
  end

  defp standardize_param(param), do: param

  def standardize_params(params) when is_map(params) do
    params
    |> Map.to_list()
    |> Enum.map(&standardize_param/1)
  end
end
